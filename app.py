...
def build_actuals(matches):
    ...
    def round_settled(cur_round):
        # o turda maçı OYNANMIŞ (sonucu belli) takımlar = kazanan ∪ kaybeden
        s = set()
        for m in by_round.get(cur_round, []):
    ...
    if len(adv) == 8:                                    # openfootball 8 üçüncüyü de yazdıysa
        actual["best_third"] = adv
    elif thirds:
    ...
    # Bir tura "çıkanlar" = bir ÖNCEKİ turun maç kazananları (sonuçlardan, günlük güncellenir)
    actual["round_set"]["Son 16"] = round_winners("Son 32")
    actual["round_set"]["Ceyrek Final"] = round_winners("Son 16")
    actual["round_set"]["Yari Final"] = round_winners("Ceyrek Final")
    actual["round_set"]["Final"] = round_winners("Yari Final")
    fin = round_winners("Final")
    if fin:
        actual["champion"] = next(iter(fin))

    # "Belli olmuş" takımlar = o tura giden maçı oynanmış olanlar (kırmızı/nötr ayrımı için)
    all_teams = {r["tr"] for rs in standings.values() for r in rs}
    actual["round_settled"]["Son 32"] = all_teams                  # gruplar bitti -> hepsi belli
    actual["round_settled"]["Son 16"] = round_settled("Son 32")
    actual["round_settled"]["Ceyrek Final"] = round_settled("Son 16")
...
def _pair_card(title, teams, reached=None, settled=None, pairset=None):
    ...
    for i in range(0, len(teams) - 1, 2):
        ...
        if pairset and frozenset({a, b}) in pairset:
            ...
            box = ""
        units += (f'<div class="matchup {box}"><span class="mt {mark(a)}">{a}</span>'
                  f'<span class="vs">–</span><span class="mt {mark(b)}">{b}</span></div>')
...
if not who:                                  # hiçbiri seçili değilse lideri göster
    who = rows[0]["Oyuncu"]
st.caption("Gruplar — rozet: yeşil ilk 2, mavi en iyi 3., sönük elenen · "
           "isim: yeşil doğru, kırmızı yanlış sıra.\n\n"
           "Eşleşmeler: takım yeşil = o tura ulaştı, kırmızı = ulaşamadı; "
